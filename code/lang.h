...
#define lang_base_alias_type_list(macro, ...) \
    ...
    macro(lang_type_and_value, __VA_ARGS__) \
...
ast_node * begin_node(lang_parser *parser, ast_node *node, ast_node_type node_type, string location_text = {}, bool update_location_on_end = false) // , u32 node_location_index = -1, bool node_location_is_done = true
{
    ...
    *location = {};
    ...
    if (!first_token.base)
    {
        assert(!first_token.count);
        first_token.base = parser->last_location_token.base;
    }
    
    location->text = first_token;
#else
    
    //if (node_location_index != -1)
...
ast_number * new_number_u64(lang_parser *parser, string location_text, u64 value, ast_node *parent = null) // , u32 node_location_index, u64 value) //
{
...
bool parse_number(parsed_number *result, lang_parser *parser, bool parsed_minus)
{
    ...
    if (count)
    {
        ...
        //value.bit_count_power_of_two = 6;
...
ast_node * parse_string_or_character_literal(lang_parser *parser)
{
    ...
    if (ok)
    {
        if (try_consume(parser, s("[")))
        {
            ...
            new_local_node(number);
            number->value.u64_value = text.base[value.u64_value];
            number->value.bit_count_power_of_two = 3;
...
ast_node * parse_base_expression(lang_parser *parser, complete_type_info type)
{
    ...
    while (expression)
    {
        ...
        if (try_consume(parser, s(".")))
        {
            ...
            {
                new_local_node(dereference, parser->node_locations.base[expression->index].text, false);
...
ast_variable * parse_declaration(lang_parser *parser)
{
    ...
    variable->can_expand = try_consume_keyword(parser, s("expand"));
...
ast_node * parse_scoped_statements(bool *ok, lang_parser *parser)
{
    if (!try_consume(parser, s("{")))
    {
        *ok = false;
    ...
    *ok = true;
    auto first_statement = lang_require_call(parse_statements(parser));
    assert(first_statement);
    lang_require(try_consume(parser, s("}")), parser->iterator, "expected '}' after scope statements");
...
{
    bool ok;
    auto first_statement = lang_require_call(parse_scoped_statements(&ok, parser));
    if (!ok)
    {
    ...
    *ok = true;
    ...
    if (token.count)
    {
        ...
        else if (token == s("if"))
        {
            ...
            branch->first_true_statement = lang_require_call(parse_single_or_scoped_statements(parser));
        ...
        else if (token == s("while"))
        {
            ...
            loop->first_statement = lang_require_call(parse_single_or_scoped_statements(parser));
        ...
        else if (token == s("loop"))
        {
            ...
            loop_with_counter->first_statement = lang_require_call(parse_single_or_scoped_statements(parser));
        ...
        else if (token == s("switch"))
        {
            ...
            lang_require_consume(";", " after switch condition");
            ...
            while (try_consume_keyword(parser, s("case")))
            {
                ...
                branch_switch_case->first_statement = lang_require_call(parse_single_or_scoped_statements(parser));
            ...
            {
                branch_switch->first_default_case_statement = lang_require_call(parse_single_or_scoped_statements(parser));
    ...
    parser->error = true;
    auto builder = &parser->error_messages;
    parser_message(parser, parser->iterator, "Error: expected statment.");
...
parse_statements_declaration
{
    ...
    if (parser->pending_comment.count || !first_statement)
    {
...
ast_node_table make_node_table(usize count, u32 value_byte_count)
{
    assert(((count - 1) & count) == 0, "table count must be power of 2");
    
    ast_node_table table = {};
    table.count            = count;
    table.value_byte_count = value_byte_count;
    
    usize byte_count = count * (sizeof(ast_node *) + value_byte_count);
    table.keys   = (ast_node **) platform_allocate_bytes(byte_count).base;
    table.values = (u8 *) (table.keys + count);

    memset(table.keys, 0, byte_count);
    
    return table;
}

void free_table(ast_node_table *table)
{
    platform_free_bytes((u8 *) table->keys);
    *table = {};
}

struct try_get_slot_result
{
    usize slot;
    bool ok;
};

try_get_slot_result try_get_slot(ast_node_table *table, ast_node *node)
{
    assert(node);
    auto iterator = get_slot_begin(table->count, (usize) node / sizeof(ast_base_node));

    usize slot;
    bool found = false;
    while (get_slot_next(&slot, &iterator))
    {
        if (!table->keys[slot] || table->keys[slot] == node)
            return { slot, true };
    }
...
u8 * insert(ast_node_table *table, ast_node *node, bool must_be_new = true)
{
    auto result = try_get_slot(table, node);
    if (!result.ok)
    {
        auto new_table = make_node_table(table->count << 1, table->value_byte_count);
        
        for (usize i = 0; i < table->count; i++)
        {
            if (table->keys[i])
            {
                auto result = try_get_slot(&new_table, table->keys[i]);
                assert(result.ok);
                new_table.keys[result.slot] = table->keys[i];
                memcpy(&new_table.values[result.slot], &table->values[i], table->value_byte_count);
            }
        }
        
        auto result = try_get_slot(&new_table, node);
        assert(result.ok);
    ...
    assert(!must_be_new || !table->keys[result.slot]);
    
    table->keys[result.slot] = node;
    
    return table->values + result.slot * table->value_byte_count;
}

struct ast_node_table_value
{
    u8 *value;
    bool ok;
};

ast_node_table_value try_get_value(ast_node_table *table, ast_node *node)
{
    auto result = try_get_slot(table, node);
    if (result.ok && table->keys[result.slot])
        return { table->values + result.slot * table->value_byte_count, true };
    else
        return {};
}

u8 * get_value(ast_node_table *table, ast_node *node)
{
    auto result = try_get_slot(table, node);
    assert(result.ok && table->keys[result.slot]);
    return table->values + result.slot * table->value_byte_count;
}

ast_node * clone(lang_parser *parser, ast_node *root, ast_node *clone_parent)
{
    auto table = make_node_table(1024, sizeof(ast_node *));
    ...
    // clone all nodes and hash node to clone
    {
        ...
        while (next(&node, &queue))
        {
            ...
            *(ast_node **) insert(&table, node) = new_node;
        }
    }
    
    {
        ...
        while (next(&entry, &queue))
        {
            ...
            auto value = try_get_value(&table, parent);
            if (value.ok)
            {
                auto cloned_parent = *(ast_node **) value.value;
                auto node = *entry.node_field;
                
                auto value = try_get_value(&table, node);
                if (value.ok)
                {
                    auto cloned_node = *(ast_node **) value.value;
                    ...
                    if (node->next)
                    {
                        auto value = try_get_value(&table, node->next);
                        if (value.ok)
                        {
                            auto cloned_next = *(ast_node **) value.value;
    ...
    auto cloned_root = *(ast_node **) get_value(&table, root);
...
bool evaluate(parsed_number *out_value, ast_node *expression)
{
    switch (expression->node_type)
    {
        ...
        case ast_node_type_name_reference:
        {
            ...
            if (!is_node_type(name_reference->reference, constant))
                return false;
...
get_array_item_count_declaration
{
    ...
    if (!array_type->item_count)
    {
        ...
        assert(ok && !value.is_float && !value.is_signed);
...
types_are_compatible_declaration
{
    ...
    {
        // implicit cast to lang_type_and_value
        if ((to.name_type.node == get_type(parser, lang_base_type_lang_type_and_value).name_type.node) && ((to.name_type.node != from.name_type.node) || from.name_type.indirection_count))
    ...
    else if (is_node_type(to.base_type.node, array_type) && is_node_type(from.base_type.node, array_type))
    {
        ...
        //MAKE implict cast to u8[] possible!
...
maybe_add_cast_declaration
{
    ...
    {
        new_local_node(compound_literal, parser->node_locations.base[expression->index].text);
        compound_literal->type        = get_type(parser, lang_base_type_lang_type_and_value);
        compound_literal->node.parent = parent;
        compound_literal->node.next   = expression->next;
        ...
        
        {
            ...
            // TODO: handle literals or constants
            // don't take pointer on a pointer
            if (!expression_type.name_type.indirection_count)
            {
                ...
                take_reference->type.name_type = expression_type.name_type;
            ...
            cast->type = get_type(parser, lang_base_type_u8);
    ...
    switch (base_type->node_type)
    {
        ...
        case ast_node_type_array_type:
        {
            ...
            if (array_type->item_count_expression)
            {
                ...
                bool ok = evaluate(&value, array_type->item_count_expression);
                assert(ok);
                count_and_alignment.byte_count *= value.u64_value;
                return count_and_alignment;
            }
            else
            {
...
{
    auto it = get_slot_begin(table.count, hash_of(key));
    
    usize slot;
    while (get_slot_next(&slot, &it))
    {
...
bool _insert(resolve_table *table, string name, ast_node *scope, ast_node *value)
{
    resolve_table_key key = { name, scope };
...
bool _insert(resolve_table *table, string name, ast_node *node)
{
    return _insert(table, name, get_parent_scope(node), node);
}
...
ast_node * find_node(lang_resolver *resolver, ast_node *at_node, string name)
{
    auto file = get_file_scope(at_node);
    
    auto scope = get_parent_scope(at_node);
    for (; !is_node_type(scope, module); scope = get_parent_scope(scope))
    {
        auto node = find_node_in_scope(resolver, scope, name);
        if (node)
            return node;
    ...
    {
        {
            ...
            auto node = find_node_in_scope(resolver, get_base_node(module), name);
            if (node)
                return node;
        }
...
struct function_call_arguments_match_result
{
    bool match;
    bool unresolved;
};

function_call_arguments_match_result function_call_arguments_match(lang_parser *parser, ast_variable **function_argument_it, ast_node **call_expression_it)
{
    auto function_argument = *function_argument_it;
    auto call_expression = *call_expression_it;
    
    assert(call_expression);
    
    defer {
        *function_argument_it = function_argument;
        *call_expression_it   = call_expression;
    };
    ...
    auto call_expression_type = get_expression_type(parser, call_expression);
    if (!call_expression_type.base_type.node)
        return { false, true };
            
    while (function_argument && !types_are_compatible(parser, function_argument->type, call_expression_type))
    {
        bool skip_argument = false;
    
        if (function_argument->can_expand)
        {
            auto base_type = function_argument->type.base_type.node;
            switch (base_type->node_type)
            {
                cases_complete;
                ...
                case ast_node_type_array_type:
                {
                    ...
                    usize item_count = 0;
                    while (types_are_compatible(parser, array_type->item_type, call_expression_type))
                    {
                        item_count++;
                        
                        call_expression = call_expression->next;
                        if (!call_expression)
                            break;
                    }
        ...
        if (!skip_argument && function_argument->default_expression)
            skip_argument = true;
...
ast_function * find_matching_function(lang_parser *parser, ast_function_overloads *function_overloads, ast_node *first_call_argument)
{
    ...
    for (auto function_reference = function_overloads->first_function_reference; function_reference; function_reference = (ast_function_reference *) function_reference->node.next)
    {
        ...
        auto call_expression = first_call_argument;
        while (call_expression)
        {
            auto result = function_call_arguments_match(parser, &function_argument, &call_expression);
...
function_overload_iterator find_matching_function_begin(lang_resolver *resolver, ast_node *at_node, string name)
{
    function_overload_iterator iterator = {};
    iterator.resolver = resolver;
    iterator.name     = name;
...
bool find_matching_function_next(ast_function_overloads **out_function_overloads, function_overload_iterator *iterator)
{
    auto resolver = iterator->resolver;
    auto name     = iterator->name;
    
    while (iterator->state < 3)
    {
        switch (iterator->state)
        {
            case 0:
            {
                ...
                {
                    iterator->state++;
                    iterator->module = iterator->file->module;
                    break;
                }
                
                *out_function_overloads = (ast_function_overloads *) get(resolver->table, name, iterator->scope);
                iterator->scope = get_parent_scope(iterator->scope);//->parent;
            ...
            case 1:
            {
                assert(iterator->module);
                *out_function_overloads = (ast_function_overloads *) get(resolver->table, name, get_base_node(iterator->module));
        
                iterator->module_reference = iterator->file->first_module_dependency;
                iterator->state++;
            ...
            case 2:
            {
                ...
                *out_function_overloads = (ast_function_overloads *) get(resolver->table, name, get_base_node(module));
...
ast_function * find_matching_function(lang_resolver *resolver, ast_node *at_node, string name, ast_node *first_call_argument)
{
    auto iterator = find_matching_function_begin(resolver, at_node, name);
    
    ast_function_overloads *function_overloads;
    while (find_matching_function_next(&function_overloads, &iterator))
    {
        auto matching_function = find_matching_function(resolver->parser, function_overloads, first_call_argument);
...
ast_function * find_matching_function(lang_resolver *resolver, ast_node *at_node, string name, ast_function_type *function_type)
{
    auto iterator = find_matching_function_begin(resolver, at_node, name);
...
void resolve(lang_parser *parser)
{
    ...
    // collect declarations
    {
        for_bucket_item(bucket, index, parser->alias_type_buckets)
        {
            ...
            auto is_new = _insert(&resolver.table, alias_type->name, get_base_node(alias_type));
            assert(is_new);
        }
        ...
        for_bucket_item(bucket, index, parser->variable_buckets)
        {
            ...
            auto is_new = _insert(&resolver.table, variable->name, get_base_node(variable));
            assert(is_new);
        }
        ...
        for_bucket_item(bucket, index, parser->constant_buckets)
        {
            ...
            auto is_new = _insert(&resolver.table, constant->name, get_base_node(constant));
            assert(is_new);
        }
        ...
        for_bucket_item(bucket, index, parser->function_buckets)
        {
            ...
            if (!function_overloads)
            {
                ...
                auto is_new = _insert(&resolver.table, function_overloads->name, parent, get_base_node(function_overloads));
                assert(is_new);
            }
    ...
    // resolve
    {
        ...
        while (did_resolve_entry)
        {
            ...
            for (u32 i = 0; i < unresolved_unary_operations.count; i++)
            {
                ...
                auto matching_function = find_matching_function(&resolver, node, ast_unary_operator_names[unary_operator->operator_type], unary_operator->expression);
                if (matching_function) 
                {
            ...
            for (u32 i = 0; i < unresolved_binary_operations.count; i++)
            {
                ...
                assert(binary_operator->left->next && !binary_operator->left->next->next);
                auto matching_function = find_matching_function(&resolver, node, ast_binary_operator_names[binary_operator->operator_type], binary_operator->left);
                if (matching_function) 
                {
            ...
            for (u32 i = 0; i < unresolved_get_function_references.count; i++)
            {
                ...
                auto matching_function = find_matching_function(&resolver, node, get_function_reference->name, function_type);
                if (matching_function) 
                {
            ...
            for (u32 i = 0; i < unresolved_function_calls.count; i++)
            {
                ...
                if (is_node_type(function_call->expression, name_reference))
                {
                    ...
                    auto matching_function = find_matching_function(&resolver, node, name_reference->name, function_call->first_argument);
                    if (matching_function) 
                ...
                if (function_type->input.base_type.node)
                {
                    ...
                    while (call_argument && function_argument)
                    {
                        ...
                        //lang_require_return_value(call_type.base_type.node, , parser->node_locations.base[call_argument->index].text, "argument type not resolved in function call for argument '%.*s'", fs(function_argument->name));
                        
                        auto compatibility = types_are_compatible(parser, input_type, call_type);
                        if (!compatibility && function_argument->can_expand)
                        {
                            auto base_type = function_argument->type.base_type.node;
                            switch (base_type->node_type)
                            {
                                ...
                                case ast_node_type_array_type:
                                {
                                    ...
                                    auto first_expression = call_argument;
                                    
                                    usize item_count = 0;
                                    while (types_are_compatible(parser, array_type->item_type, call_type))
                                    {
                                        item_count++;
                                    ...
                                    if (item_count)
                                    {
                                        auto location = get_node_location(parser, first_expression);
                                        
                                        new_local_leaf_node(array_literal, location);
                                        array_literal->node.parent = get_base_node(function_call);
                                        
                                        auto base_type_with_count = clone(parser, base_type, get_base_node(array_literal));
                                        auto array_type_with_count = get_node_type(array_type, base_type_with_count);
                                        array_type_with_count->item_count_expression = get_base_node(new_number_u64(parser, location, item_count, base_type_with_count));
                                        
                                        array_literal->type = to_type(parser, base_type_with_count);
                                        array_literal->item_count = item_count;
                                        array_literal->first_expression = first_expression;
                                        
                                        auto tail = &first_expression;
                                        for (usize i = 0; i < item_count; i++)
                                        {
                                            first_expression->parent = get_base_node(array_literal);
                                            tail = &first_expression->next;
                                            first_expression = *tail;
                                        }
                                        
                                        *call_argument_tail_next = get_base_node(array_literal);
                                        array_literal->node.next = *tail;
                                        *tail = null;
                                        
                                        call_argument = get_base_node(array_literal);
                                        maybe_add_cast(parser, &call_argument, type_compatibility_requires_cast, function_argument->type);
                        ...
                        if (!compatibility && try_add_default_argument(parser, function_call, function_type, &call_argument_tail_next, &function_argument))
                            continue;
                    ...
                    {
                        lang_require_return_value(function_argument->default_expression, , parser->node_locations.base[function_call->node.index].text, "expected argument '%.*s' in function call", fs(function_argument->name));
                        
                        bool ok = try_add_default_argument(parser, function_call, function_type, &call_argument_tail_next, &function_argument);
                        assert(ok);
                    }
                }
    ...
    // semantic checks
    {
        ...
        for (u32 i = 0; i < unresolved_names.count; i++)
        {
            ...
            if (!name_reference->reference && !is_node_type(node->parent, function_call))
            {
        ...
        for (u32 i = 0; i < unresolved_function_calls.count; i++)
        {
            ...
            // HACK:
            if (name_reference->name == s("sprintf_s"))
                continue;
            ...
            auto iterator = find_matching_function_begin(&resolver, node, name_reference->name);
            ...
            if (first_function)
                print_line(error_messages, "no functions called '%.*s' found.", fs(name_reference->name));
            else
                error_messages->indent--;
        }
        ...
        for_bucket_item(bucket, index, parser->array_literal_buckets)
        {
            ...
            {
                auto type = get_expression_type(parser, it);
                lang_require_return_value(type.base_type.node, , parser->node_locations.base[it->index].text, "array literal item type is not resolved");
                
                auto compatibility = types_are_compatible(parser, item_type, type);
                lang_require_return_value(compatibility, , parser->node_locations.base[it->index].text, "array literal item type is not compatible with array literal type");
                    
                maybe_add_cast(parser, &it, compatibility, item_type);
            }
        }
...
ast_node * add_table_entry_pointer(lang_parser *parser, ast_constant *table, ast_variable *base_type, u32 index, string location_text)
{
    ...
    new_local_named_node(cast, unary_operator, location_text);
...
void generate_type_info_tables(lang_parser *parser)
{
    ...
    // type table literal
    {
        ...
        // number_type array
        {
            ...
            array_type->item_count_expression = get_base_node(new_number_u64(parser, parser->node_locations.base[array_type->node.index].text, entry_count, get_base_node(array_type)));
    ...
    // number table
    {
        ...
        defer
        {
            array_type->item_count_expression = get_base_node(new_number_u64(parser, location_text, type_index, get_base_node(array_type)));
    ...
    // compound & compound_field table
    {
        ...
        defer 
        {
            compound_table_type->item_count_expression = get_base_node(new_number_u64(parser, compound_location_text, compound_index, get_base_node(compound_table_type)));
            compound_table->item_count = compound_index;
            
            compound_field_table_type->item_count_expression = get_base_node(new_number_u64(parser, compound_location_text, compound_field_index, get_base_node(compound_field_table_type)));
    ...
    // enumeration & enumeration item
    {
        ...
        defer 
        {
            enumeration_table_type->item_count_expression = get_base_node(new_number_u64(parser, enumeration_location_text, enumeration_index, get_base_node(enumeration_table_type)));
            enumeration_table->item_count = enumeration_index;
            
            enumeration_item_table_type->item_count_expression = get_base_node(new_number_u64(parser, enumeration_location_text, enumeration_item_index), get_base_node(enumeration_item_table_type));
